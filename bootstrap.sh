...
configure_environment() {
    ...
    if [ ! -d "$ENV_DIR" ]; then
        ...
        if [[ $REPLY =~ ^[Yy]$ ]]; then
            ...
            info "Creating GCS bucket '$BUCKET_NAME' for Terraform state..."; gsutil mb -l europe-west1 -p "$GCP_PROJECT_ID" "gs://${BUCKET_NAME}" || warn "Bucket 'gs://${BUCKET_NAME}' may already exist. Continuing..."
        fi
...
trigger_builds() {
    ...
    info "Triggering backend build..."; gcloud builds triggers run "${BE_SERVICE_NAME}-trigger" --branch="$GITHUB_BRANCH" --project="$GCP_PROJECT_ID" --region="europe-west1"
    info "Triggering frontend build..."; gcloud builds triggers run "$GCP_PROJECT_ID-trigger" --branch="$GITHUB_BRANCH" --project $GCP_PROJECT_ID --region="europe-west1"
